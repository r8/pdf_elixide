...
`PdfElixide.Document.search/2` finds every occurrence of a pattern in a
document's text and reports where each one sits on the page, as
...
across:
  ...
  * There are **no backreferences and no lookaround** — `(?=…)`, `(?<=…)` and
    `\1` do not compile. In exchange, matching is linear in the length of the
    text, so no pattern can hang the search.
...
gives back the box of the entire line. There is no narrower answer available.
...
**A match can cross a line.** A page's runs are joined with a single space before
matching, and no newline is inserted, so a phrase split across two lines still
matches as one. The page behaves as a single line: `^` and `$` anchor to the page
rather than to a line, and `.` never stops at a line end.
...
the pattern**, is served from it, so searching the same document for twenty terms
costs one pass over the text rather than twenty.
...
limit, so only the pages it actually visited are indexed.
